...
# ANOVAs
for (dv in c('willing_friend', 'willing_romantic', 'wtp_logged', 'true', 'talk_listen', 'not_judge', 'emotion', 'understand', 'body')) {
    print(paste0("*-*-* ", dv, "*-*-*"))

    formula <- as.formula(paste(dv, "~ as.factor(agent_cond) * as.factor(interact_cond)"))
    mod <- aov(formula, data = d_merged)
    print(summary(mod))
    anova_stats(mod)
}
...
# t-tests...
for (dv in c('willing_friend', 'willing_romantic', 'wtp_logged', 'true', 'talk_listen', 'not_judge', 'emotion', 'understand', 'body')) {
    print("Participants were more willing when provided a trial than when not in human cond")
    x <- d_merged[d_merged$interact_cond == 'yes' & d_merged$agent_cond == 'human', dv]
    y <- d_merged[d_merged$interact_cond == 'no' & d_merged$agent_cond == 'human', dv]
    vart <- var.test(x, y)
    ...
    print("Participants were more willing when provided a trial than when not in AI cond")
    x <- d_merged[d_merged$interact_cond == 'yes' & d_merged$agent_cond == 'ai', dv]
    y <- d_merged[d_merged$interact_cond == 'no' & d_merged$agent_cond == 'ai', dv]
    vart <- var.test(x, y)
    tt <- t.test(x, y, paired = FALSE, var.equal = vart$p.value > 0.05)
    print(tt)
    print(cohen.d(x, y))
...
create_plot <- function(data, y_var, title, a_names, interaction_conds, ylim = c(1, 110)) {
  p <- ggplot(data, aes(x = factor(agent_cond), y = !!sym(y_var), fill = factor(interact_cond)), color = factor(interact_cond)) +
    theme_bw() +
...
plot_titles <- c("Willingness to\nFind a Friend", "Willingness to\nFind Romantic Partner", "Willingness to Pay", "Not True")
y_vars <- c("willing_friend", "willing_romantic", "wtp_logged", "true")
ylims <- list(c(1, 110), c(1, 110), c(0, 6), c(1, 110))

# Create the plots
plots <- lapply(1:4, function(i) {
  create_plot(d_merged, y_vars[i], plot_titles[i], a_names, interaction_conds, ylims[[i]])
})

# Arrange the plots
figure <- ggarrange(plotlist = plots[1:3], nrow = 1, ncol = 3, common.legend = TRUE, legend = "top", vjust = 1.0, hjust = 0.5)
...
ggsave(paste0(fname, ".pdf"), last_plot(), dpi = 500, width = 15 - 15/4, height = 5)

######### PLOT MEDIATORS #########


# Function to create the plot
create_plot <- function(data, y_var, title, a_names, interaction_conds) {
  p <- ggplot(data, aes(x = factor(agent_cond), y = !!sym(y_var), fill = factor(interact_cond)), color = factor(interact_cond)) +
    theme_bw() +
...
#Not mutual, understand, feel, body
# Variables for the plots
plot_titles <- c("Not True", "Not Judge", "Available", "Unable to\nUnderstand", "Unable to\nFeel", "Cannot Be\nPhysically Intimate")
y_vars <- c("true", "not_judge", "not_judge", "understand", "emotion", "body")

# Create the plots
plots <- lapply(1:6, function(i) {
  create_plot(d_merged, y_vars[i], plot_titles[i], a_names, interaction_conds)
})

# Arrange the plots
figure <- ggarrange(plotlist = plots, nrow = 2, ncol = 3, common.legend = TRUE, legend = "top", vjust = 1.0, hjust = 0.5)

# Annotate and save the figure
annotate_figure(figure, left = text_grob("Mean", color = "black", face = "plain", size = 26, rot = 90),
                bottom = text_grob("Agent Condition", color = "black", face = "plain", size = 26, margin(b = 2), hjust = 0.33))

ggsave(paste0(fname, "_mediators.pdf"), last_plot(), dpi = 500, width = 15 - 15/4, height = 10)
...
d_merged$agent_cond_n <- as.numeric(as.factor(d_merged$agent_cond))
d_merged$interact_cond_n <- ifelse (d_merged$interact_cond == 'yes', 1, 2)

# agent_cond_n -> "human" = 2, "ai" = 1
# trial -> "yes" = 1, "no": 2

# Try serial mediation:
############ ON WILLING_FRIEND ############
process(data = d_merged, y = "wtp_logged", x = "agent_cond_n",
...
# MODERATION ON THE 'IV - FIRST MEDIATOR' PATH
############ ON WILLING_FRIEND ############
process(data = d_merged, y = "wtp_logged", x = "agent_cond_n",
        m = c("true", "willing_friend"), w = "interact_cond_n", model = 83, effsize = 1, total = 1, stand = 1,
        contrast = 1, boot = 10000, modelbt = 1, seed = 654321)

process(data = d_merged, y = "wtp_logged", x = "agent_cond_n",
        m = c("not_judge", "willing_friend"), w = "interact_cond_n", model = 83, effsize = 1, total = 1, stand = 1,
        contrast = 1, boot = 10000, modelbt = 1, seed = 654321)

process(data = d_merged, y = "wtp_logged", x = "agent_cond_n",
        m = c("talk_listen", "willing_friend"), w = "interact_cond_n", model = 83, effsize = 1, total = 1, stand = 1,
        contrast = 1, boot = 10000, modelbt = 1, seed = 654321)

############ ON WILLING_ROMANTIC ############
process(data = d_merged, y = "wtp_logged", x = "agent_cond_n",
        m = c("true", "willing_romantic"), w = "interact_cond_n", model = 83, effsize = 1, total = 1, stand = 1,
        contrast = 1, boot = 10000, modelbt = 1, seed = 654321)

process(data = d_merged, y = "wtp_logged", x = "agent_cond_n",
        m = c("not_judge", "willing_romantic"), w = "interact_cond_n", model = 83, effsize = 1, total = 1, stand = 1,
        contrast = 1, boot = 10000, modelbt = 1, seed = 654321)

process(data = d_merged, y = "wtp_logged", x = "agent_cond_n",
        m = c("talk_listen", "willing_romantic"), w = "interact_cond_n", model = 83, effsize = 1, total = 1, stand = 1,
        contrast = 1, boot = 10000, modelbt = 1, seed = 654321)
